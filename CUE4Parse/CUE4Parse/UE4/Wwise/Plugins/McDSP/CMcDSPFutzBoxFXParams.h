// Ported from CUE4Parse/UE4/Wwise/Plugins/McDSP/CMcDSPFutzBoxFXParams.cs
#pragma once
...
{
    enum class FutzFilterSlope : int32_t
    {
        eFutzFilterSlope12 = 0x0,
        eFutzFilterSlope24 = 0x1
    };

    enum class FutzDistortionMode : int32_t
    {
    ...
    enum class FutzBitDepthType : int32_t
    {
    ...
    // 164 speaker, handset and radio impulse models, numbered 0x00..0xA3 with no gaps.
    enum class FutzSIMType : int32_t
    {
    ...
    struct McDSPFutzFiltersFXParams
    {
        bool bEnable = false;
        FutzFilterSlope LPFSlope = static_cast<FutzFilterSlope>(0);
        float fLPFFreq = 0;
        float fLPFQ = 0;
        FutzFilterSlope HPFSlope = static_cast<FutzFilterSlope>(0);
        float fHPFFreq = 0;
        float fHPFQ = 0;

        McDSPFutzFiltersFXParams() = default;

        explicit McDSPFutzFiltersFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
            LPFSlope = Ar.Read<FutzFilterSlope>();
            fLPFFreq = Ar.Read<float>();
            fLPFQ = Ar.Read<float>();
            HPFSlope = Ar.Read<FutzFilterSlope>();
            fHPFFreq = Ar.Read<float>();
            fHPFQ = Ar.Read<float>();
        }
    };

    struct McDSPFutzDistortionFXParams
    {
        bool bEnable = false;
        FutzDistortionMode iMode = static_cast<FutzDistortionMode>(0);
        float fAmount = 0;
        float fIntensity = 0;
        float fRectify = 0;

        int32_t DistortionIntensityMode = 0;
        float DistortionWobble = 0;
        float DistortionChopAmount = 0;
        int32_t DistortionChopMode = 0;

        McDSPFutzDistortionFXParams() = default;

        explicit McDSPFutzDistortionFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
            ...
            if (Ar.Version >= 150)
            {
                DistortionIntensityMode = Ar.Read<int32_t>();
                DistortionWobble = Ar.Read<float>();
                DistortionChopAmount = Ar.Read<float>();
                DistortionChopMode = Ar.Read<int32_t>();
            }
        }
    };

    struct McDSPFutzEQFXParams
    {
        bool bEnable = false; // not in 150
        FutzEQType FilterType = static_cast<FutzEQType>(0);
        float fFreq = 0;
        float fQ = 0;
        float fGain = 0;

        McDSPFutzEQFXParams() = default;

        explicit McDSPFutzEQFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
            FilterType = Ar.Read<FutzEQType>();
            fFreq = Ar.Read<float>();
            fQ = Ar.Read<float>();
            fGain = Ar.Read<float>();
        }
    ...
    struct McDSPFutzNoiseFXParams
    {
        ...
        McDSPFutzNoiseFXParams() = default;

        explicit McDSPFutzNoiseFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
            fLevel = Ar.Read<float>();
            fLPFFreq = Ar.Read<float>();
            fHPFFreq = Ar.Read<float>();
            fThresh = Ar.Read<float>();
            fRange = Ar.Read<float>();
            fRecovery = Ar.Read<float>();
        }
    };

    struct McDSPFutzSIMFXParams
    {
        bool bEnable = false;
        FutzSIMType iType = static_cast<FutzSIMType>(0);
        float fTuning = 0;

        McDSPFutzSIMFXParams() = default;

        explicit McDSPFutzSIMFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
            iType = Ar.Read<FutzSIMType>();
            fTuning = Ar.Read<float>();
        }
    };

    struct McDSPFutzGateFXParams
    {
        bool bEnable = false;
        float fThreshold = 0;
        float fRange = 0;
        ...
        McDSPFutzGateFXParams() = default;

        explicit McDSPFutzGateFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
    ...
    struct McDSPFutzLoFiFXParams
    {
        bool bEnable = false;
        FutzBitDepthType iBitDepthType = static_cast<FutzBitDepthType>(0);
        uint32_t iDownSampleIndex = 0;
        float fFilter = 0;

        McDSPFutzLoFiFXParams() = default;

        explicit McDSPFutzLoFiFXParams(FWwiseArchive& Ar)
        {
            bEnable = Ar.Read<uint8_t>() != 0;
            iBitDepthType = Ar.Read<FutzBitDepthType>();
            iDownSampleIndex = Ar.Read<uint32_t>();
            fFilter = Ar.Read<float>();
        }
    };

    struct McDSPGlobalFXParams
    {
        float fInputGain = 0;
        float fOutputGain = 0;
        float fBalance = 0;

        McDSPGlobalFXParams() = default;

        explicit McDSPGlobalFXParams(FWwiseArchive& Ar)
        {
            fInputGain = DbToLinear(Ar.Read<float>());
            fOutputGain = DbToLinear(Ar.Read<float>());
            fBalance = Ar.Read<float>();
        }
    };

    struct McDSPFutzBoxFXParams
    {
        McDSPFutzFiltersFXParams Filters;
        McDSPFutzDistortionFXParams Distortion;
        McDSPFutzEQFXParams EQ;
        McDSPFutzNoiseFXParams Noise;
        McDSPFutzSIMFXParams SIM;
        McDSPFutzGateFXParams Gate;
        McDSPFutzLoFiFXParams LoFi;
        McDSPGlobalFXParams Global;
        // A plugin-local version, and it trails the payload rather than leading it. Unrelated to the
        // bank version on the archive.
        int32_t Version = 0;

        McDSPFutzBoxFXParams() = default;

        // Member init order is declaration order, which is also the wire order here.
        explicit McDSPFutzBoxFXParams(FWwiseArchive& Ar)
            : Filters(Ar), Distortion(Ar), EQ(Ar), Noise(Ar), SIM(Ar), Gate(Ar), LoFi(Ar), Global(Ar)
        {
            Version = Ar.Version >= 150 ? Ar.Read<int32_t>() : 0;
        }
    };

    class CMcDSPFutzBoxFXParams : public IAkPluginParam
    {
    public:
        McDSPFutzBoxFXParams Params;

        explicit CMcDSPFutzBoxFXParams(FWwiseArchive& Ar) : Params(Ar) {}
    };
}

// Ported from CUE4Parse/UE4/Versions/FAssetRegistryVersion.cs
// Version used for serializing asset registry caches, both runtime and editor.
//
// The odd one out of the FXxxVersion family: instead of an enum + GUID + a Get(FArchive) that guesses from
// the game, it is a standalone version enum plus a helper that reads a leading GUID off the archive and only
// then trusts the following version int.
//
// Deliberate differences from C#:
//   * FAssetRegistryVersionType is a scoped enum with an explicit int32_t underlying type. C#'s plain enum is
//     int-backed and already written `FAssetRegistryVersionType.X` at every call site, so scoping costs
//     nothing and keeps ~25 member names out of the surrounding namespace.
//   * C#'s `out` parameter becomes a reference parameter.
#pragma once
...
{
    enum class FAssetRegistryVersionType : int32_t
    {
    ...
    namespace FAssetRegistryVersion
    {
        namespace
        {
            const Objects::Core::Misc::FGuid GUID(0x717F9EE7, 0xE9B0493A, 0x88B39132, 0x1B388107);
        }

        inline void TrySerializeVersion(Readers::FArchive& Ar, FAssetRegistryVersionType& version)
        {
            const auto guid = Ar.Read<Objects::Core::Misc::FGuid>();

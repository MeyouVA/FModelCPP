// Ported from CUE4Parse/UE4/Versions/FUE5ReleaseStreamObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FUE5ReleaseStreamObjectVersion
    {
        ...
        inline const FGuid GUID(0xD89B5E42, 0x24BD4D46, 0x8412ACA8, 0xDF641779);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_BlackMythWukong) return StoreReflectionCaptureEncodedHDRDataInRG11B10Format;
            if (game < GAME_UE5_0) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE5_1) return SerializeFloatPinDefaultValuesAsSinglePrecision;
            if (game < GAME_UE5_3) return LinkedAnimGraphMemberReference;
            if (game < GAME_UE5_4) return OpenColorIODisabledDisplayConfigurationDefault;
            if (game < GAME_UE5_6) return OrthographicAutoNearFarPlane;
            if (game < GAME_UE5_7) return RecalculateClothAssetSerializedBounds;
            if (game < GAME_UE5_8) return AudioSectionPlayUntilFinished;

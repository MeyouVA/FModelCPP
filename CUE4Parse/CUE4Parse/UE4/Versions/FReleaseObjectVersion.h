// Ported from CUE4Parse/UE4/Versions/FReleaseObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FReleaseObjectVersion
    {
        ...
        inline const FGuid GUID(0x9C54D522, 0xA8264FBE, 0x94210746, 0x61B482D0);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_AssaultFireFuture) return FFieldPathOwnerSerialization;
            if (game == GAME_DarkPicturesAnthologyManofMedan || game == GAME_DarkPicturesAnthologyLittleHope || game == GAME_DarkPicturesAnthologyTheDevilinMe) return GeometryCollectionCacheRemovesMassToLocal;
            if (game < GAME_UE4_11) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE4_13) return StaticMeshExtendedBoundsFix;
            if (game < GAME_UE4_14) return LevelTransArrayConvertedToTArray;
            if (game < GAME_UE4_15) return AddComponentNodeTemplateUniqueNames;
            if (game < GAME_UE4_16) return SpeedTreeBillboardSectionInfoFixup;
            if (game < GAME_UE4_17) return SkyLightRemoveMobileIrradianceMap;
            if (game < GAME_UE4_19) return RenameNoTwistToAllowTwistInTwoBoneIK;
            if (game < GAME_UE4_20) return AddSkeletalMeshSectionDisable;
            if (game < GAME_UE4_21) return MeshDescriptionNewSerialization;
            if (game < GAME_UE4_23) return TrailNodeBlendVariableNameChange;
            if (game < GAME_UE4_24) return Unused_SoundClass2DReverbSend;
            if (game < GAME_UE4_25) return DeprecateFilmbackSettings;
            if (game < GAME_UE4_26) return FFieldPathOwnerSerialization;
            if (game < GAME_UE4_27) return StructureDataAddedToConvex;
            if (game < GAME_UE5_0) return LonglatTextureCubeDefaultMaxResolution;

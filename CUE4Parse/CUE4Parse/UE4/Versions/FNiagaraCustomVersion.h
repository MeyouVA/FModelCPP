// Ported from CUE4Parse/UE4/Versions/FNiagaraCustomVersion.cs
...
{
    namespace FNiagaraCustomVersion
    {
        ...
        inline const FGuid GUID(0xFCF57AFA, 0x50764283, 0xB9A9E658, 0xFFA02D32);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game < GAME_UE4_21) return EmitterLocalSpaceLiteralConstant;
            if (game < GAME_UE4_23) return SkelMeshInterfaceAPIImprovements;
            if (game < GAME_UE4_24) return AddLibraryAssetProperty;
            if (game < GAME_UE4_25) return DisableSortingByDefault;
            if (game < GAME_UE4_26) return StandardizeParameterNames;
            if (game < GAME_UE4_27) return SignificanceHandlers;
            if (game < GAME_UE5_0) return MoveDefaultValueFromFNiagaraVariableMetaDataToUNiagaraScriptVariable;
            if (game < GAME_UE5_1) return StaticSwitchFunctionPinsUsePersistentGuids;
            if (game < GAME_UE5_2) return EventSpawnsUpdateInitialAttributeValues;
            if (game < GAME_UE5_3) return DynamicPinNodeFixup;
            if (game < GAME_UE5_4) return RibbonRendererLinkOrderDefaultIsUniqueID;
            if (game < GAME_UE5_5) return NDCSpawnGroupOverrideDisabledByDefault;
            if (game < GAME_UE5_6) return EmitterStateAddLoopDelayEnabled;
            if (game < GAME_UE5_7) return AddedNewInterpolatedSpawnMode;

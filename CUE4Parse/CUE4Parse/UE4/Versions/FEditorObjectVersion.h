// Ported from CUE4Parse/UE4/Versions/FEditorObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FEditorObjectVersion
    {
        ...
        inline const FGuid GUID(0xE4B068ED, 0xF49442E9, 0xA231DA0B, 0x2E46BB41);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_TEKKEN7) return ComboBoxControllerSupportUpdate;
            if (game == GAME_Paragon) return AddedMaterialSharedInputs;
            if (game < GAME_UE4_12) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE4_13) return GatheredTextPackageCacheFixesV1;
            if (game < GAME_UE4_14) return SplineComponentCurvesInStruct;
            if (game < GAME_UE4_15) return RefactorMeshEditorMaterials;
            if (game < GAME_UE4_16) return AddedInlineFontFaceAssets;
            if (game < GAME_UE4_17) return MaterialThumbnailRenderingChanges;
            if (game < GAME_UE4_19) return GatheredTextEditorOnlyPackageLocId;
            if (game < GAME_UE4_20) return AddedMorphTargetSectionIndices;
            if (game < GAME_UE4_21) return SerializeInstancedStaticMeshRenderData;
            if (game < GAME_UE4_22) return MeshDescriptionNewAttributeFormat;
            if (game < GAME_UE4_23) return MeshDescriptionRemovedHoles;
            if (game < GAME_UE4_24) return RemoveLandscapeHoleMaterial;
            if (game < GAME_UE4_25) return SkeletalMeshBuildRefactor;
            if (game < GAME_UE4_26) return SkeletalMeshMoveEditorSourceDataToPrivateAsset;

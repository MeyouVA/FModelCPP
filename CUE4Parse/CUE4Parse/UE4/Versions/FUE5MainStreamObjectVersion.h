// Ported from CUE4Parse/UE4/Versions/FUE5MainStreamObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FUE5MainStreamObjectVersion
    {
        ...
        inline const FGuid GUID(0x697DD581, 0xE64f41AB, 0xAA4A51EC, 0xBEB7B628);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_BlackMythWukong) return UseTriangleMeshBVH;
            if (game < GAME_UE5_0) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE5_1) return TextureDoScaleMipsForAlphaCoverage;
            if (game < GAME_UE5_2) return WorldPartitionActorDescActorAndClassPaths;
            if (game < GAME_UE5_3) return RigVMGeneratedClass;
            if (game < GAME_UE5_4) return ClothCollectionSingleLodSchema;
            if (game < GAME_UE5_6) return VisualLoggerAddedSeparateWorldTime;
            if (game < GAME_UE5_8) return RuntimeVirtualTextureMaterialValidation;

// Ported from CUE4Parse/UE4/Versions/FFrameworkObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FFrameworkObjectVersion
    {
        ...
        inline const FGuid GUID(0xCFFC743F, 0x43B04480, 0x939114DF, 0x171D2073);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_TEKKEN7) return WheelOffsetIsFromWheel;
            if (game < GAME_UE4_12) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE4_13) return FixNonTransactionalPins;
            if (game < GAME_UE4_14) return RemoveSoundWaveCompressionName;
            if (game < GAME_UE4_15) return GeometryCacheMissingMaterials;
            if (game < GAME_UE4_16) return ChangeAudioComponentOverrideSubtitlePriorityDefault;
            if (game < GAME_UE4_17) return HardSoundReferences;
            if (game < GAME_UE4_18) return LocalVariablesBlueprintVisible;
            if (game < GAME_UE4_19) return UserDefinedStructsBlueprintVisible;
            if (game < GAME_UE4_20) return FunctionTerminatorNodesUseMemberReference;
            if (game < GAME_UE4_22) return EditableEventsUseConstRefParameters;
            if (game < GAME_UE4_24) return BlueprintGeneratedClassIsAlwaysAuthoritative;
            if (game < GAME_UE4_25) return EnforceBlueprintFunctionVisibility;
            if (game < GAME_UE4_26) return StoringUCSSerializationIndex;

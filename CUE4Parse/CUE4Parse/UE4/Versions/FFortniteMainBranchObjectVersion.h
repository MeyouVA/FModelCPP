// Ported from CUE4Parse/UE4/Versions/FFortniteMainBranchObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FFortniteMainBranchObjectVersion
    {
        ...
        inline const FGuid GUID(0x601D1886, 0xAC644F84, 0xAA16D3DE, 0x0DEAC7D6);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_Aion2) return WorldPartitionHLODActorDescSerializeSourceHLODLayer;
            if (game < GAME_UE4_20) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE4_21) return CachedMaterialQualityNodeUsage;
            if (game < GAME_UE4_22) return FoliageLazyObjPtrToSoftObjPtr;
            if (game < GAME_UE4_23) return FortHUDElementNowRequiresTag;
            if (game < GAME_UE4_24) return SupportVirtualBoneInRetargeting;
            if (game < GAME_UE4_26) return AnimLayerGuidConformation;
            if (game < GAME_UE4_27) return ChaosSolverPropertiesMoved;
            if (game < GAME_UE5_0) return RemoveLandscapeWaterInfo;
            if (game < GAME_UE5_1) return GravityOverrideDefinedInWorldSpace;
            if (game < GAME_UE5_2) return WorldPartitionHLODActorDescSerializeStats;
            if (game < GAME_UE5_3) return WorldPartitionHLODActorUseSourceCellGuid;
            if (game < GAME_UE5_4) return WaterBodyStaticMeshFixup;
            if (game < GAME_UE5_5) return SkeletalHalfEdgeData;
            if (game < GAME_UE5_6) return SolverIterationsDataSupportInChaosVisualDebugger;
            if (game < GAME_UE5_7) return PCGChangedSurfaceSamplerDefaultGridCreationMode;
            if (game < GAME_UE5_8) return LandscapeAdvancedWeightBlending;

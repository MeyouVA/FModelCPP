// Ported from CUE4Parse/UE4/Versions/FRenderingObjectVersion.cs
...
namespace CUE4Parse::UE4::Versions
{
    namespace FRenderingObjectVersion
    {
        ...
        inline const FGuid GUID(0x12F88B9F, 0x88754AFC, 0xA67CD90C, 0x383ABD29);
        ...
        inline Type Get(Readers::FArchive& Ar)
        {
            ...
            if (game == GAME_TEKKEN7) return MapBuildDataSeparatePackage;
            if (game < GAME_UE4_12) return BeforeCustomVersionWasAdded;
            if (game < GAME_UE4_13) return CustomReflectionCaptureResolutionSupport;
            if (game < GAME_UE4_14) return IntroducedMeshDecals;
            if (game < GAME_UE4_16) return FixedBSPLightmaps;
            if (game < GAME_UE4_17) return ShaderResourceCodeSharing;
            if (game < GAME_UE4_18) return AddedbUseShowOnlyList;
            if (game < GAME_UE4_19) return VolumetricLightmaps;
            if (game < GAME_UE4_20) return ShaderPermutationId;
            if (game < GAME_UE4_21) return IncreaseNormalPrecision;
            if (game < GAME_UE4_22) return VirtualTexturedLightmaps;
            if (game < GAME_UE4_23) return GeometryCacheFastDecoder;
            if (game < GAME_UE4_24) return VirtualTexturedLightmapsV2;
            if (game < GAME_UE4_25) return MaterialShaderMapIdSerialization;
            if (game < GAME_UE4_26) return AutoExposureDefaultFix;
            if (game < GAME_UE4_27) return VolumeExtinctionBecomesRGB;
            if (game < GAME_UE5_3) return VirtualTexturedLightmapsV3;
            if (game < GAME_UE5_4) return NaniteForceMaterialUsage;
            if (game < GAME_UE5_5) return AddedMaterialExpressionIncludesHash;
